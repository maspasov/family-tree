...
/** Flip the UI to Bulgarian (selectors below are Bulgarian). No-op if already BG. */
export async function forceBg(page: Page) {
  const bg = page.getByRole('button', { name: 'БГ', exact: true })
  if (await bg.isVisible().catch(() => false)) {
    await bg.click()
    await expect(page.getByText(/Родословни дървета|Родословно дърво|Дървото е празно/).first())
      .toBeVisible()
  }
}
...
export async function importJson(page: Page, rows: unknown) {
  ...
  await expect(dialog.getByText(/Готово\. Записани \d+ записа/)).toBeVisible({ timeout: 120_000 })
...
export async function deleteTree(page: Page, slug: string) {
  ...
  await page.waitForURL((u) => u.hash === '#/' || u.hash === '', { timeout: 20_000 })
}
