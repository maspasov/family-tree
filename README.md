...
- 🔒 **Само поканени** — сайтът изисква вход с Google и показва дървото само на
  акаунти от списъка `config/app.editors`. Всеки друг вижда единствено екран за
  вход (и съобщение „Нямате достъп“, ако влезе с непоканен акаунт).
- ✏️ Всеки поканен акаунт може и да разглежда, и да редактира.
...
Файлът [`firestore.rules`](./firestore.rules) заключва всичко: четене и писане
на дървото само за акаунти от списъка `config/app.editors`. Единствено
`config/app` (самият списък) остава публично четим, за да може приложението да
провери дали влезлият акаунт е поканен.
...
- Поле: `editors` — тип **array** — стойности: имейлите (Google) на поканените,
  напр. `martospasov@gmail.com`, `dayana.r.krusteva@gmail.com`

Добавяте/махате хора по всяко време, като редактирате този масив — промяната
важи веднага (без повторно качване), както за екрана за вход, така и за
самите Firestore правила (`firestore.rules` проверява същия списък).
