...
3. **Email Templates → Create New Template** → използвайте следните
   ...
   > ⚠️ Честа грешка: в редактора на шаблона има отделно поле **„To Email“**
   > (обикновено над Subject/Content, не в тялото на писмото) — трябва да
   > съдържа точно `{{to_email}}`. Ако е оставено с фиксиран адрес (напр.
   > имейла на вашия EmailJS акаунт по подразбиране), писмата ще стигат
   > винаги до него, а не до добавения човек, независимо какво е `to_email`
   > в кода.
...
### 8b. Известие до администратора — по избор

Освен известието до добавения човек, приложението може да изпраща и тихо
известие до администратора при всяко добавяне на нов човек (независимо дали
той има имейл). Използва същите Service ID/Public Key, но собствен шаблон:

1. **Email Templates → Create New Template** (втори шаблон) → полето
   **„To Email“** — вашият собствен имейл (фиксиран текст, не `{{to_email}}`
   този път). Променливи в тялото:
   - `{{person_name}}` — името на добавения човек
   - `{{person_email}}` — неговият имейл (може да е празно)
   - `{{tree_name}}`, `{{family_name}}`, `{{site_url}}` — както по-горе
2. Копирайте **Template ID** и попълнете в `.env.local`:

   ```
   VITE_EMAILJS_ADMIN_TEMPLATE_ID=...
   VITE_EMAILJS_ADMIN_EMAIL=you@gmail.com
   ```

Без тези две променливи приложението просто пропуска известието до
администратора — известието до добавения човек (по-горе) работи независимо.
